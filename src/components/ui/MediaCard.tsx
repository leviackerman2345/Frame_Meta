...
export function MediaCard({
  ...
  return (
    ...
    >
      ...
      {isCatalog && rating && (
        <div className="absolute top-3 right-3 z-20 flex items-center gap-1 px-2 py-1 rounded-lg bg-[#121212]/95 border border-white/10 shadow-lg">
      ...
      {!isCatalog && badge && (
        <div className="absolute top-4 right-4 z-20">
          <span className={`inline-block px-2.5 py-1 text-[7px] md:text-[9px] font-black tracking-[0.2em] bg-[#121212]/95 border rounded-full uppercase shadow-2xl transition-colors duration-300 ${getBadgeStyles(badge)}`}>
            {badge}
          </span>
        </div>
      )}
      ...
      <div className="absolute inset-0 transition-opacity flex flex-col justify-end p-5 items-center z-10 bg-gradient-to-t from-black/95 via-black/40 to-transparent text-center">
        ...
        {isCatalog && badge && (
          <span className={`inline-block px-2.5 py-1 mb-3 text-[7px] md:text-[9px] font-black tracking-[0.2em] bg-[#121212]/95 border rounded-full uppercase shadow-sm w-max mx-auto transition-colors duration-300 ${getBadgeStyles(badge)}`}>
            {badge}
          </span>
        )}
