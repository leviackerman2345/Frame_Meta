...
export function Navbar() {
  ...
  return (
    <nav className="fixed top-6 left-1/2 z-[100] w-full max-w-6xl -translate-x-1/2 px-4">
      ...
      <div className="flex w-full items-center justify-between rounded-[2rem] bg-zinc-950/80 border border-white/10 px-3 py-3 shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
          ...
          <AnimatePresence mode="wait">
            {!isSearchActive ? (
              <motion.div 
                ...
                className="hidden md:flex items-center space-x-1 rounded-full bg-zinc-900/80 border border-white/5 px-2 py-2"
              >
                {navLinks.map((link) => {
              ...
              <motion.form 
                ...
                transition={{ duration: 0.4, ease: [0.16, 1, 0.3, 1] }}
                className="flex items-center w-full max-w-2xl bg-zinc-900/90 border border-white/20 rounded-full px-6 relative"
              >
                ...
                <AnimatePresence>
                  {showSuggestions && suggestions.length > 0 && (
                    <motion.div
                      ...
                      ref={suggestionsRef}
                    >
                      <div className="overflow-hidden rounded-[2rem] bg-zinc-950/90 border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
                        <div className="py-3">
        ...
        <div className="flex items-center space-x-2 md:space-x-3 pr-2">
          {!isSearchActive && (
            ...
            >
              ...
              <a
                href="https://github.com/leviackerman2345/Frame_Meta"
                target="_blank"
                rel="noopener noreferrer"
                className="flex h-10 w-10 md:h-11 md:w-11 items-center justify-center rounded-xl bg-zinc-800 text-white hover:bg-zinc-700 transition-all border border-white/10"
                aria-label="GitHub"
              >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="md:hidden mt-4 w-full rounded-[2rem] bg-zinc-900/95 border border-white/10 p-4 shadow-2xl overflow-hidden"
            id="mobile-nav"
          >
