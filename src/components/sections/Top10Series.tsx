...
export async function Top10Series() {
  const series = await getPopularTVSeries(10);
    ...
    <section className="w-full max-w-7xl mx-auto px-6 md:px-12 py-10 relative z-20">
      <div className="mb-8">
        <h2 className="text-2xl font-semibold text-white tracking-tight drop-shadow-sm">
          {top10SeriesHeading.title}
        </h2>
        {top10SeriesHeading.subtitle && (
          <p className="text-sm text-zinc-400/90 mt-1.5 font-medium">{top10SeriesHeading.subtitle}</p>
        )}
      </div>
          ...
          >
            ...
            <div className="absolute inset-0 transition-opacity flex flex-col justify-end p-5 z-20 bg-gradient-to-t from-black/90 via-black/40 to-transparent">
              ...
              <div className="flex items-center gap-1.5 mt-2 text-[10px] md:text-xs text-white/70 font-medium tracking-wide w-full overflow-hidden whitespace-nowrap">
                ...
                {item.year && (item.genre || item.rating) && <span className="opacity-50 text-[8px] shrink-0">•</span>}
                {item.genre && <span className="truncate min-w-0">{item.genre}</span>}
                {item.genre && item.rating && <span className="opacity-50 text-[8px] shrink-0">•</span>}
                {item.rating && (
